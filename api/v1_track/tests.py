...
class TrackAPIViewTest(APITestCase):
    ...
    def test_permission(self):
        response = self.client.get('/api/v1/tracks/', follow=True)
        not_expected = {'detail': 'Authentication credentials were not provided.'}
...
class TrackSerializerTest(APITestCase):
    ...
    def test_if_we_can_post_audio_file(self, mock_save):
        ...
        response = self.client.post('/api/v1/tracks/', data=payload, follow=True)
    ...
    def test_if_passing_not_audio_file(self):
        ...
        response = self.client.post('/api/v1/tracks/', data=payload, follow=True)
        self.assertIn('This field must be supplied with an audio file', response.content.decode())
